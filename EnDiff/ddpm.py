...
#定义位置嵌入模块（正弦位置嵌入和随机或可学习的正弦位置嵌入模块）
...
class GaussianDiffusion(Module):
    ...
    def p_losses(self, x_start, t, noise = None, offset_noise_strength = None):
        ...
        if self.objective == 'pred_noise':
            ...
            pred_noise = self.predict_noise_from_start(x, t, x_start_pred)
        elif self.objective == 'pred_v':
            v = model_out
            x_start_pred = self.predict_start_from_v(x, t, v)
            pred_noise = self.predict_noise_from_start(x, t, x_start_pred)
        # land loss
        r = torch.flatten(x_start_pred, 1)
        h = torch.flatten(x_start, 1)
        r = F.log_softmax(r, dim=-1)
        h = F.log_softmax(h, dim=-1)
        land_loss = F.kl_div(r, h, log_target=True, reduction='batchmean') * self.land_loss_weight

        # 原有的损失计算方式 (uw_loss)
        uw_loss = F.mse_loss(model_out, target, reduction='none')
        uw_loss = reduce(uw_loss, 'b ... -> b', 'mean')* self.uw_loss_weight
        uw_loss = uw_loss * extract(self.loss_weight, t, uw_loss.shape)
        uw_loss = uw_loss.mean()
