...
BASE_CMD_PLACEHOLDER = (
    "torchrun --nproc_per_node={num_gpu} --master-port {port}"
...
# ------------------ 空闲判定 ------------------ #
def gpu_is_idle(idx: str, mem_ratio: float = 0.6, util_max: int = 15, duration: int = 5) -> bool:
...
TASK_COMMAND_QUEUE.clear()

def main():
    logger.info("=" * 25 + f" 共 {len(TASK_COMMAND_QUEUE)} 个排队任务，完成后自动退出 " + "=" * 25)
    occupied = set()
    busy_counter = 0
